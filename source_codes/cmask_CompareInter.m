...
%             masksF(:,:,i) = imread(strcat('/Users/nikitatripathi/Desktop/NDI-IRIS-DATA-NOTRE-DAME/Output/NormalizedMasks/', classes(f), '/', num2str(i), '_mano.bmp'));
            masksF(:,:,i) = imread(strcat('./Common_Masks/', classes(f), '_mano.bmp'));
        end
        for i=1:num
...
%             masksS(:,:,i) = imread(strcat('/Users/nikitatripathi/Desktop/NDI-IRIS-DATA-NOTRE-DAME/Output/NormalizedMasks/', classes(s), '/', num2str(i), '_mano.bmp'));
            masksS(:,:,i) = imread(strcat('./Common_Masks/', classes(s), '_mano.bmp'));
        end

        for f1=1:num
            for s1=1:num
                % use the same mask for two codes (since assumption is we
                % compare two images of same iris)
                scores = [scores matchCodes(codesF(:,:,:,f1), codesS(:,:,:,s1), masksF(:,:,f1), masksF(:,:,f1), l);];
            end
        end
