...
if shared
    num = 80;
    for i=1:num
        im = imread(['/Users/nikitatripathi/Desktop/NDI-IRIS-DATA-NOTRE-DAME/Output/NormalizedImages/00000/' num2str(i) '_imno.bmp']);
        codes(:,:,:,i) = extractCode(im,ICAtextureFilters);
        masks(:,:,i) = imread(['/Users/nikitatripathi/Desktop/NDI-IRIS-DATA-NOTRE-DAME/Output/NormalizedMasks/00000/' num2str(i) '_mano.bmp']);
    end
else
    if intraclass
        for i=1:num
            im = imread(['/Users/nikitatripathi/Desktop/NDI-IRIS-DATA-NOTRE-DAME/Output/NormalizedImages/04560/' num2str(i) '_imno.bmp']);
            codes(:,:,:,i) = extractCode(im,ICAtextureFilters);
            masks(:,:,i) = imread(['/Users/nikitatripathi/Desktop/NDI-IRIS-DATA-NOTRE-DAME/Output/NormalizedMasks/04560/' num2str(i) '_mano.bmp']);
        end
    else
        i = 1;
        num = numel(classes);
        for c=classes
            im = imread(strcat('/Users/nikitatripathi/Desktop/NDI-IRIS-DATA-NOTRE-DAME/Output/NormalizedImages/', c, '/3_imno.bmp'));
            codes(:,:,:,i) = extractCode(im,ICAtextureFilters);
            masks(:,:,i) = imread(strcat('/Users/nikitatripathi/Desktop/NDI-IRIS-DATA-NOTRE-DAME/Output/NormalizedMasks/', c, '/3_mano.bmp'));
            i = i+1;
        end
    end
end
...
if ~shared
    for f = 1:num % First code for comparison
        ...
        for s = (f+1):num % Second code for comparison
            score = matchCodes(codes(:,:,:,f), codes(:,:,:,s), masks(:,:,f), masks(:,:,s), l);
            scores = [scores score];
    %         disp(['Comparison score of 04201d' num2str(f+361) ' and 04201d' num2str(s+361) ': ' num2str(score)])
        end
    end
else
    for f = 1:(num/2) % First code for comparison
        disp(["Starting with " num2str(f)])
        for s = (f+1):num % Second code for comparison
            score = matchCodes(codes(:,:,:,f), codes(:,:,:,s), masks(:,:,f), masks(:,:,s), l);
            scores = [scores score];
