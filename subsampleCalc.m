function avg_min_entropy = subsampleCalc(k)
    ...
    path = "/Users/nikitatripathi/Desktop/NDI-IRIS-DATA-NOTRE-DAME/"; % Where iris codes are stored
    ...
    for f=1:cls
        ...
        for s=f+1:cls
            for i=1:num
                im = imread(strcat(path, 'Output/NormalizedImages/', classes(f), '/', num2str(i), '_imno.bmp'));
                codesF(:,:,:,i) = extractCode(im,ICAtextureFilters);
                masksF(:,:,i) = imread(strcat(path, 'Output/NormalizedMasks/', classes(f), '/', num2str(i), '_mano.bmp'));

                im = imread(strcat(path, 'Output/NormalizedImages/', classes(s), '/', num2str(i), '_imno.bmp'));
                codesS(:,:,:,i) = extractCode(im,ICAtextureFilters);
                masksS(:,:,i) = imread(strcat(path, 'Output/NormalizedMasks/', classes(s), '/', num2str(i), '_mano.bmp'));
            end
            ...
            for f1=1:num
                for s1=1:num
                    ...
                    for t=1:trials
                        [cPos, mPos] = unmaskedPos(mF, mS, k);                                      % Generate k positions
                        scores(t, pos) = matchCodes(cF(cPos), cS(cPos), mF(mPos), mS(mPos), l);     % Match scores for those positions
                    end
                    pos = pos+1;
